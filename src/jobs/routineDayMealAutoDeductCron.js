// jobs/routineMealAutoDeductCron.js
...
async function processDueRoutineMealDeductions() {
  ...
  console.log(`[ROUTINE-CRON] BD Time: ${bdNow.toLocaleTimeString()} | Day: ${todayDayName} | Date: ${todayDateStr}`);

  const docs = await UserDayWiseRoutineMeal.find({
    "meals.day": todayDayName,
    "meals.is_on": true,
  });

  console.log(`[ROUTINE-CRON] Found ${docs.length} document(s) with active meals today`);
  ...
  for (const doc of docs) {
    ...
    for (const meal of doc.meals) {
      ...
      if (startMinutes === null) {
        console.error(`[ROUTINE-CRON] ⚠️ Invalid start_time "${meal.start_time}" for ${meal.meal_type} (user ${doc.user_id}) — skipping`);
        continue;
      }
      ...
      console.log(
        `[ROUTINE-CRON] Checking ${meal.meal_type} | start_time(raw): ${meal.start_time} | correctedStartMinutes: ${startMinutes} | dueMinutes: ${dueMinutes} | currentMinutes: ${currentMinutes} | willDeduct: ${currentMinutes >= dueMinutes}`
      );
      ...
      try {
        ...
        if (!userDoc || userDoc.balance < amount) {
          console.log(`[ROUTINE-CRON] ❌ Insufficient balance for user ${doc.user_id}, turning meal OFF`);
          meal.is_on = false;
          ...
          console.log(`[ROUTINE-CRON] ✅ Deducted ${amount} from user ${doc.user_id}, credited to institute ${doc.institute_id}`);
        ...
        await session.abortTransaction();
        console.error(`[ROUTINE-CRON] Deduction failed for ${doc.user_id} / ${meal.meal_type}:`, err.message);
      } finally {
        session.endSession();
      }
    }
...
cron.schedule("* * * * *", () => {
  processDueRoutineMealDeductions().catch((err) => console.error("Routine deduct cron error:", err));
});

module.exports = { processDueRoutineMealDeductions };