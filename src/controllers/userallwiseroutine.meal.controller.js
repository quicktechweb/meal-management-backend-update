...
const allwiseRoutineCreateUserMeal = async (req, res) => {
  try {
    ...
    const existingDayWiseMeal = await UserDayWiseRoutineMeal.findOne({
      ...
      const conflictPairs = existingDayWiseMeal.meals.filter(
        (m) =>
          m.is_on === true &&
...
const allwiseFingerprintAttend = async (req, res) => {
  try {
    ...
    const presentResult = await UserAllWiseRoutineMeal.updateMany(
      {
    ...
    const expiredResult = await UserAllWiseRoutineMeal.updateMany(
      {
        "meals.day": todayDayName,
...
const allwiseRoutineGetUserMeal = async (req, res) => {
  const user = req.user;
...
const allwiseRoutineGetAllMealsById = async (req, res) => {
  const { id } = req.params;
  try {
    const allWiseMealList = await UserAllWiseRoutineMeal.findOne({
      uid: id,
...
const allwiseRoutineGetInsituteUserMeal = async (req, res) => {
  const user = req.user;
